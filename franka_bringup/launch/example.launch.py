...
default_mjcf_file = os.path.join(
    franka_description_share, 'mjcf', 'robots', 'fr3', 'fr3.xml.xacro'
)
...
def generate_robot_nodes(context):
    ...
    use_mujoco_default = LaunchConfiguration('use_mujoco').perform(context)
    load_gripper_arg = LaunchConfiguration('load_gripper').perform(context)
    mjcf_file_arg = LaunchConfiguration('mjcf_file').perform(context)
    configs = load_yaml(config_file)
    nodes = []
    for item_name, config in configs.items():
        namespace = config['namespace']
        use_mujoco = str(config.get('use_mujoco', use_mujoco_default))
        load_gripper = load_gripper_arg if load_gripper_arg.lower() != 'config' else str(config['load_gripper'])
        mjcf_file = mjcf_file_arg if mjcf_file_arg else str(config.get('mjcf_file', ''))
        if use_mujoco.lower() == 'true' and not mjcf_file:
            mjcf_file = default_mjcf_file
        nodes.append(
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'mjcf_file',
            default_value='',
            description='Path to MuJoCo scene xacro (use_mujoco=true, empty=use package default)',
        ),
        DeclareLaunchArgument(
            'load_gripper',
            default_value='config',
            description='Override gripper loading ("true", "false", or "config")',
        ),
