...
franka::RobotState Robot::readOnceActiveControl() {
  ...
  try {
    ...
    auto logger = rclcpp::get_logger("franka_hardware_robot");
    RCLCPP_ERROR(logger, "Control exception during readOnce: %s", e.what());
    try {
      automaticErrorRecovery();
    } catch (const std::exception& recovery_error) {
      RCLCPP_ERROR(logger, "Automatic error recovery failed: %s", recovery_error.what());
      return current_state_;
    }

    if (!restartActiveControl()) {
      RCLCPP_ERROR(logger, "Failed to restart active control after recovery.");
      return current_state_;
    }
    ...
    try {
      ...
      RCLCPP_ERROR(logger, "readOnce after recovery failed: %s", retry_error.what());
      return current_state_;
    }
  } catch (const std::exception& e) {
    auto logger = rclcpp::get_logger("franka_hardware_robot");
    RCLCPP_ERROR(logger, "readOnceActiveControl failed: %s", e.what());
    return current_state_;
  }
}

bool Robot::restartActiveControl() {
  active_control_.reset();
  ...
  try {
    ...
    if (joint_velocity_interface_active_) {
      active_control_ = robot_->startJointVelocityControl(
          research_interface::robot::Move::ControllerMode::kJointImpedance);
      return true;
    }
    if (joint_position_interface_active_) {
    ...
    auto logger = rclcpp::get_logger("franka_hardware_robot");
    RCLCPP_ERROR(logger, "Restarting active control failed: %s", e.what());
  }
