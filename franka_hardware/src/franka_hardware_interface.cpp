...
const std::string kPrefixName = "prefix";
...
namespace franka_hardware 
{
    ...
    FrankaHardwareInterface::FrankaHardwareInterface(const std::shared_ptr<Robot>& robot, const std::string& arm_id)
    ...
    : command_interfaces_info_({
        ...
        {k_HW_IF_CARTESIAN_POSE_COMMAND, hw_cartesian_pose_commands_.size(), pose_cartesian_interface_claimed_},
    }) 
    {
    ...
    CallbackReturn FrankaHardwareInterface::on_init(const hardware_interface::HardwareInfo& info) 
    {
        ...
        {
            std::cout << "Parameter 'version' is not set. Please update your URDF (aka franka_description)." << std::endl;
            logRclcppFatalRed(getLogger(), "Parameter '%s' is not set. Please update your URDF (aka franka_description).", kVersionName.c_str());
            return CallbackReturn::ERROR;
        }
        ...
        auto itp = info_.hardware_parameters.find(kPrefixName);
        user_prefix_ = (itp != info_.hardware_parameters.end()) ? itp->second : "";

        if (!user_prefix_.empty()) name_stem_ = user_prefix_ + "_" + arm_id_;
        else                       name_stem_ = arm_id_;
    ...
    CallbackReturn FrankaHardwareInterface::on_activate(const rclcpp_lifecycle::State& /*previous_state*/) 
    {
        ...
        const bool comm_violation =
            hw_franka_robot_state_.current_errors.communication_constraints_violation ||
            hw_franka_robot_state_.last_motion_errors.communication_constraints_violation;
        if (comm_violation && !comm_constraints_recovery_sent_)
        {
            RCLCPP_WARN(getLogger(),
                        "communication_constraints_violation detected; sending automatic error recovery.");
            try 
            {
            ...
            {
                RCLCPP_ERROR(getLogger(), "Automatic error recovery failed: %s", e.what());
            }
            catch (const std::exception& e)
            {
                RCLCPP_ERROR(getLogger(), "Automatic error recovery failed: %s", e.what());
            }
            comm_constraints_recovery_sent_ = true;
        }
    ...
    CallbackReturn FrankaHardwareInterface::on_deactivate(const rclcpp_lifecycle::State& /*previous_state*/) 
    {
        ...
        robot_->stopRobot();
        comm_constraints_recovery_sent_ = false;
        RCLCPP_INFO(getLogger(), "Stopped");
        return CallbackReturn::SUCCESS;
    }

    hardware_interface::return_type FrankaHardwareInterface::read(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/) 
    {
    ...
    hardware_interface::return_type FrankaHardwareInterface::write(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/) 
    {
        ...
        else if (velocity_cartesian_interface_running_ && elbow_command_interface_running_ && !first_elbow_update_) 
        {
        ...
        else if (pose_cartesian_interface_running_ && elbow_command_interface_running_ && !first_cartesian_pose_update_ && !first_elbow_update_) 
        {
            // Wait until the first read pass after robot controller is activated to write the elbow
            // command to the robot
    ...
    std::vector<StateInterface> FrankaHardwareInterface::export_state_interfaces() 
    {
        ...
        state_interfaces.emplace_back(StateInterface(name_stem_, k_robot_state_interface_name, reinterpret_cast<double*>(&hw_franka_robot_state_addr_))); // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
        state_interfaces.emplace_back(StateInterface(name_stem_, k_robot_model_interface_name, reinterpret_cast<double*>(&hw_franka_model_ptr_))); // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)

        // cartesian pose state interface 16 element pose matrix
        for (auto i = 0U; i < 16; i++) 
        {
            const std::string pose_name = (user_prefix_.empty()) ? user_prefix_ + "_" + std::to_string(i) : std::to_string(i);
            state_interfaces.emplace_back(StateInterface(pose_name, k_HW_IF_CARTESIAN_POSE_STATE, &cartesian_pose_state_.at(i)));
        }

        // elbow state interface
        for (auto i = 0U; i < elbow_state_names_.size(); i++) 
        {
            const std::string elbow_name = (user_prefix_.empty()) ? user_prefix_ + "_" + elbow_state_names_.at(i) : elbow_state_names_.at(i);

            state_interfaces.emplace_back(
                StateInterface(elbow_name, k_HW_IF_ELBOW_STATE, &elbow_state_.at(i)));
        }

        state_interfaces.emplace_back(StateInterface(name_stem_, "robot_time", &robot_time_state_));
    ...
    std::vector<CommandInterface> FrankaHardwareInterface::export_command_interfaces() 
    {
        ...
        for (auto joint_index = 0U; joint_index < info_.joints.size(); joint_index++) 
        {
            ...
            for (const auto& command_interface : joint.command_interfaces) 
            {
                command_interfaces.emplace_back(
                    CommandInterface(joint.name, command_interface.name, &command_interface_map_.at(command_interface.name)[joint_index])
                );
        ...
        for (const auto& gpio : info_.gpios) 
        {
            for (const auto& command_interface : gpio.command_interfaces) 
            {
                ...
                command_interfaces.emplace_back(
                    CommandInterface(gpio.name, command_interface.name, &command_interface_map_.at(command_interface.name)[vector_index])
                );
    ...
    hardware_interface::return_type FrankaHardwareInterface::prepare_command_mode_switch(const std::vector<std::string>& start_interfaces, const std::vector<std::string>& stop_interfaces)
    {
        auto contains_interface_type = [this](const std::string& interface, const std::string& interface_type) 
        {
        ...
        auto generate_error_message = [this](const std::string& start_stop_command, const std::string& interface_name, size_t actual_interface_size, size_t expected_interface_size) 
        {
            std::string error_message = fmt::format("Invalid number of {} interfaces to {}. Expected {}, given {}", interface_name, start_stop_command, expected_interface_size, actual_interface_size);
        ...
        for (const auto& interface : command_interfaces_info_) 
        {
            ...
            {
                generate_error_message("start", interface.interface_type, num_start_interface, interface.size);
            }
        }
    ...
    hardware_interface::return_type FrankaHardwareInterface::perform_command_mode_switch(const std::vector<std::string>& /*start_interfaces*/, const std::vector<std::string>& /*stop_interfaces*/) 
    {
        ...
        if (elbow_command_interface_claimed_ && !(velocity_cartesian_interface_claimed_ || pose_cartesian_interface_claimed_)) 
        {
    ...
    void FrankaHardwareInterface::initializePositionCommands(const franka::RobotState& robot_state) 
    {
        ...
        initializeCommand(first_elbow_update_, elbow_command_interface_running_, hw_elbow_command_, mapped_elbow);
        auto mapped_position = std::vector<double>{robot_state.q.begin(), robot_state.q.end()};
        initializeCommand(first_position_update_, position_joint_interface_running_, hw_position_commands_, mapped_position);
        auto mapped_cartesian_pose = std::vector<double>{robot_state.O_T_EE.begin(), robot_state.O_T_EE.end()};
        initializeCommand(first_cartesian_pose_update_, pose_cartesian_interface_running_, hw_cartesian_pose_commands_, mapped_cartesian_pose);
    }
    ...
    template <typename CommandType>
    void initializeCommand(bool& first_update, const bool& interface_running, CommandType& hw_command, const CommandType& new_command) 
    {
