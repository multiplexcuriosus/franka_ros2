...
FrankaCartesianPoseInterface::FrankaCartesianPoseInterface(bool command_elbow_active,
  ...
  for (auto i = 0U; i < 16; i++) {
    const auto resource_name = resource_prefix_ + std::to_string(i);
    auto full_interface_name = resource_name + "/" + cartesian_pose_command_interface_name_;
    auto state_interface_name = resource_name + "/" + cartesian_pose_state_interface_name_;
  ...
  if (command_elbow_active_) {
    for (const auto& elbow_command_name : hw_elbow_names_) {
      auto full_elbow_command_name =
          resource_prefix_ + elbow_command_name + "/" + elbow_command_interface_name_;
    ...
    for (const auto& elbow_state_name : elbow_state_names_) {
      auto full_elbow_state_name =
          resource_prefix_ + elbow_state_name + "/" + elbow_state_interface_name_;
