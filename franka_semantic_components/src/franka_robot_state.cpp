...
FrankaRobotState::FrankaRobotState(const std::string& name,
...
FrankaRobotState::FrankaRobotState(const std::string& name,
  ...
  std::string resource_name = interface_prefix_;
  if (!resource_name.empty() && resource_name.back() == '/') {
    resource_name.pop_back();
  }
  resource_name = (resource_name.empty()) ? robot_name_ : resource_name + "_" + robot_name_;
  full_robot_state_interface_name_ = resource_name + "/" + state_interface_name_;
